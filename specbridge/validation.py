...
# Matches [JsonPropertyName("x")] ... [Description("y")] pairs within a property block.
# The non-greedy [^[]* segment stops at the next '[' so it doesn't cross into a
# different property's attributes.
_PROP_WITH_DESC_RE = re.compile(
    r'\[JsonPropertyName\("([^"]+)"\)\][^[]*\[Description\("([^"]+)"\)\]',
    re.DOTALL,
)
...
def _normalize_json_name(json_name: str) -> str:
    """Normalize a JSON field name for comparison purposes.

    Some APIs (e.g. Procore) use ``%{placeholder}`` template syntax in field
    names to denote parameterized keys (e.g. ``custom_field_%{definition_id}``).
    LLMs consistently drop the ``%`` and emit ``{definition_id}`` instead, which
    is the natural C# attribute form.  Normalizing both sides avoids spurious
    MISSING_FIELD / PHANTOM_FIELD warnings for these template fields.
    """
    return json_name.replace("%{", "{")
...
def _check_json_property_coverage(
    ...
    for te in all_types:
        ...
        for f in te.get("fields") or []:
            ...
            if not json_name:
                continue
            normalized = _normalize_json_name(json_name)
            expected = f'[JsonPropertyName("{normalized}")]'
...
def _collect_all_json_names(packet: dict[str, Any]) -> set[str]:
    ...
    Names are normalized (``%{placeholder}`` → ``{placeholder}``) so that
    PHANTOM_FIELD comparison uses the same form the LLM produces.
    """
    names: set[str] = set()
    all_types: list[dict[str, Any]] = [packet["root"]] + list(packet.get("nested_types") or [])
    for te in all_types:
        for f in te.get("fields") or []:
            ...
            if json_name:
                names.add(_normalize_json_name(json_name))
    return names
...
# ---------------------------------------------------------------------------
# Per-type validation (used by the retry loop in pipeline._generate_paginated)
# ---------------------------------------------------------------------------


def _check_single_type_declaration(
    ...
    if not name:
        return
    acceptable = {name, format_data_object_name(name)}
    if not (declared & acceptable):
        result.add(
            "error",
            "MISSING_CLASS",
...
def _check_undeclared_types_in_block(
    code: str,
    result: ValidationResult,
    known_external_types: frozenset[str] | None = None,
) -> None:
    """Like _check_undeclared_property_types but accepts external known type names.

    ``known_external_types`` contains names declared in OTHER type blocks (e.g.
    nested type names when validating the root type block). These are treated as
    valid references so root class property declarations like
    ``public ExtendedFlag? Flag { get; init; }`` do not fire UNDECLARED_TYPE when
    ``ExtendedFlag`` is a correctly named nested type generated in a separate block.
    """
    extra_known: frozenset[str] = known_external_types or frozenset()
    declared: set[str] = set(_CLASS_DECL_RE.findall(code))
    all_known = declared | extra_known
    seen_undeclared: set[str] = set()
    for type_name in _PROP_TYPE_RE.findall(code):
        if type_name in _CSHARP_PRIMITIVES:
            continue
...
def _check_pii_reason_in_description(
    code: str,
    type_entry: dict[str, Any],
    result: ValidationResult,
) -> None:
    """Detect when pii_reason text is verbatim in a [Description(...)] attribute.

    When a field has a PII review reason and no real spec description, the LLM
    sometimes copies the ``pii_reason`` (surfaced via the ``[review: ...]``
    prompt attribute) directly into ``[Description("...")]``.  This check detects
    that by matching each field's ``pii_reason`` against the ``[Description]``
    text that appears after its ``[JsonPropertyName]`` in the generated code.

    Match criteria: either an exact string match (after stripping trailing
    punctuation and whitespace) or a substantial substring match (>30 chars).
    """
    fields: list[dict[str, Any]] = type_entry.get("fields") or []
    # Build json_name → pii_reason for fields that have a reason set.
    pii_reasons: dict[str, str] = {
        f["json_name"]: f["pii_reason"]
        for f in fields
        if f.get("json_name") and f.get("pii_reason")
    }
    if not pii_reasons:
        return

    for m in _PROP_WITH_DESC_RE.finditer(code):
        json_name = m.group(1)
        desc_text = m.group(2)
        pii_reason = pii_reasons.get(json_name)
        if not pii_reason:
            continue

        pii_norm = pii_reason.rstrip(".").strip().lower()
        desc_norm = desc_text.rstrip(".").strip().lower()

        exact = pii_norm == desc_norm
        substantial_substring = (
            len(pii_norm) > 30 and (pii_norm in desc_norm or desc_norm in pii_norm)
        )
        if exact or substantial_substring:
            result.add(
                "error",
                "PII_REASON_IN_DESCRIPTION",
                f"[Description] for '{json_name}' contains PII classification text instead of the spec description",
                "pii_reason text found verbatim in [Description(...)]; use the spec description or 'No description available'",
            )


def validate_type_block(
    code: str,
    type_entry: dict[str, Any],
    known_external_types: frozenset[str] | None = None,
) -> ValidationResult:
    """Validate a single stitched type block before full assembly.

    Used by the retry loop to check one type at a time. Runs a focused subset
    of deterministic checks appropriate for a single-type code block:

      STRUCTURAL     — brace balance
      ARTIFACT       — stitching markers / template placeholders
      MISSING_CLASS  — this type's own declaration is present
      DUPLICATE_FIELD — duplicate JsonPropertyName within the block
      UNDECLARED_TYPE — property types not declared locally or as known-external

    Args:
        code: The stitched C# for this type only (all pages stitched together).
        type_entry: The processed type dict (must have "name").
        known_external_types: Type names declared in OTHER type blocks (e.g.
            nested type names when validating the root type block). These are
            treated as valid references so a correctly generated root block
            does not fire UNDECLARED_TYPE for its nested-type properties.

    Returns:
        ValidationResult with all findings. result.is_clean is True when no
        issues were found.
    """
    result = ValidationResult()
    _check_structural(code, result)
    _check_artifacts(code, result)
    _check_single_type_declaration(code, type_entry, result)
    _check_duplicate_json_properties(code, result)
    _check_undeclared_types_in_block(code, result, known_external_types)
    _check_pii_reason_in_description(code, type_entry, result)
    return result
